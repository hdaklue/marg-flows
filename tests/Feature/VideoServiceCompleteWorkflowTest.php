...
it('demonstrates the complete video service workflow as requested', function () {
    ...
    $urlEditor->trim(10, 60)->resize(new Dimension(1280, 720))->convert(new Conversion720p()); // Extract 1 minute from 10s mark // Resize to 720p // Apply 720p conversion
...
it('demonstrates batch processing workflow', function () {
    ...
    foreach ($sourceVideos as $index => $source) {
        ...
        $editor->trim(0, 60)->resize(new Dimension(1280, 720))->convert($conversion); // First minute of each // Standard size // Standard quality
