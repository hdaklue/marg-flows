...
it('demonstrates the complete video service workflow as requested', function () {
    ...
    $urlEditor
        ->trim(10, 60)  // Extract 1 minute from 10s mark
...
it('demonstrates multiple quality processing from single source', function () {
    ...
    foreach ($qualityVersions as $quality => $conversion) {
        ...
        $editor
            ...
            ->convert($conversion);  // Quality-specific conversion
    ...
    foreach ($processedVideos as $quality => $video) {
        expect($video['editor']->getSourcePath())->toBe($sourceVideo)
            ->and(count($video['operations']))->toBe(2)
            ->and($video['operations'][0]['type'])->toBe('trim')
            ->and($video['operations'][1]['type'])->toBe('convert');
...
it('demonstrates advanced processing chain with operations and saveAs', function () {
    ...
    $editor
        ...
        ->resize(new Dimension(1920, 1080))  // Upscale to Full HD
        ->crop(100, 100, new Dimension(1720, 880))  // Crop with margins
    ...
    expect($operations[0]['start'])->toBe(30.0)
        ->and($operations[0]['duration'])->toBe(180.0)
        ->and($operations[1]['dimension'])->toBeInstanceOf(Dimension::class)
        ->and($operations[2]['x'])->toBe(100)
        ->and($operations[2]['y'])->toBe(100)
        ->and($operations[3]['fps'])->toBe(30)
        ->and($operations[4]['conversion'])->toBeInstanceOf(Conversion720p::class);
...
it('demonstrates batch processing workflow', function () {
    ...
    foreach ($sourceVideos as $index => $source) {
        ...
        $editor
            ->trim(0, 60)  // First minute of each
            ->resize(new Dimension(1280, 720))  // Standard size
            ->convert($conversion);  // Standard quality
    ...
    foreach ($processedBatch as $item) {
        ...
        expect(count($operations))->toBe(3)
            ->and($item['editor']->getSourcePath())->toBe($item['source']);
...
it('demonstrates the complete API as requested in user prompt', function () {
    ...
    expect($editor1->getOperations())->toHaveCount(2)
        ->and($editor2->getOperations())->toHaveCount(2)
        ->and($editor3->getOperations())->toHaveCount(5);
