...
it('creates actual video files using FFmpeg and processes them', function () {
    ...
    try {
        $media = FFMpeg::fromDisk('local')
            ->open('videos/temp/source.mp4'); // This will be created
        ...
        if ($returnCode === 0 && file_exists($fullInputPath)) {
            ...
            expect(file_exists($fullInputPath))->toBeTrue()
                ->and(filesize($fullInputPath))->toBeGreaterThan(1000); // Should be > 1KB
            ...
            $editor->resize(new Dimension(320, 240))
            ...
            if ($processReturnCode === 0) {
                expect(file_exists($fullOutputPath))->toBeTrue()
                    ->and(filesize($fullOutputPath))->toBeGreaterThan(500); // Processed file should exist
                
                // Check that files exist in Laravel storage
                expect(Storage::disk('local')->exists($testVideoPath))->toBeTrue()
                    ->and(Storage::disk('local')->exists($outputVideoPath))->toBeTrue();
                ...
                expect($inputSize)->toBeGreaterThan(0)
                    ->and($outputSize)->toBeGreaterThan(0);
...
it('downloads real video from URL and processes it', function () {
    ...
    try {
        ...
        if ($downloadReturnCode === 0 && file_exists($fullDownloadPath) && filesize($fullDownloadPath) > 1000) {
    ...
    if (count($sourceFiles) > 0) {
        ...
        foreach ($sourceFiles as $index => $sourceFile) {
            $editor = Video::fromDisk($sourceFile)
                ->resize(new Dimension(160, 120))  // Very small
                ->convert(new Conversion480p());
        ...
        dump("✅ Created " . count($sourceFiles) . " test video files for batch processing");
        ...
        $this->markTestSkipped('Could not create test videos for batch processing');
    }
});
