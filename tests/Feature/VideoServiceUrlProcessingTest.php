...
it('can chain complex operations from URL to multiple outputs', function () {
    ...
    foreach ($processingChains as $quality => $chain) {
        expect($chain['editor']->getSourcePath())
            ->toBe($sourceUrl)
            ->and($chain['operations'][0]['type'])
            ->toBe('trim')
            ->and($chain['operations'][1]['type'])
            ->toBe('resize')
            ->and($chain['operations'][2]['type'])
            ->toBe('convert');
    }
});
...
it('simulates real-world URL processing pipeline', function () {
    ...
    $editor
        ->trim(
            $pipeline['operations']['trim']['start'],
            $pipeline['operations']['trim']['duration'],
        )
    ...
    expect($operations[1]['type'])
        ->toBe('resize')
        ->and($operations[1]['dimension'])
        ->toBeInstanceOf(Dimension::class);

    expect($operations[2]['type'])
        ->toBe('convert')
        ->and($operations[2]['conversion'])
        ->toBeInstanceOf(Conversion720p::class);
});
