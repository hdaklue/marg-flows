...
it('demonstrates complete video service API workflow', function () {
    ...
    $editor
        ->trim(10, 30)  // Trim from 10s to 30s duration
        ->resize(new Dimension(1280, 720))  // Resize to 720p dimensions
        ->convert(new Conversion720p());  // Apply 720p conversion
...
it('demonstrates multiple conversion workflow', function () {
    ...
    expect(count($results))->toBe(2)
        ->and($results[0]['conversion'])->toBe('720p')
        ->and($results[0]['bitrate'])->toBe(2500)
        ->and($results[1]['conversion'])->toBe('480p')
        ->and($results[1]['bitrate'])->toBe(1200);
});
...
it('demonstrates complex video manipulation chain', function () {
    ...
    $result = $editor
        ->trim(5, 45)  // Extract 40 seconds starting at 5s
        ->resize(new Dimension(1920, 1080))  // Resize to Full HD
        ->crop(100, 100, new Dimension(1720, 880))  // Crop with 100px margin
        ->watermark($watermarkPath, 'bottom-right', 0.8)  // Add watermark
        ->convert(new Conversion720p())  // Convert to 720p
        ->setFrameRate(30);  // Set frame rate
    
    $operations = $editor->getOperations();
    
    expect($result)->toBeInstanceOf(VideoEditor::class)
        ->and(count($operations))->toBe(6);
...
it('demonstrates dimension calculations and constraints', function () {
    ...
    $scenarios = [
        ['source' => [640, 360], 'target' => [1280, 720], 'name' => '360p to 720p'],
        ['source' => [1920, 1080], 'target' => [854, 480], 'name' => '1080p to 480p'],
        ['source' => [1280, 720], 'target' => [1280, 720], 'name' => '720p to 720p (same)'],
    ];
    ...
    foreach ($scenarios as $scenario) {
        ...
        expect($scaledDim)->toBeInstanceOf(Dimension::class)
            ->and($scaledDim->getWidth())->toBeGreaterThan(0)
            ->and($scaledDim->getHeight())->toBeGreaterThan(0);
    }
});
...
it('demonstrates conversion constraints and quality settings', function () {
    ...
    foreach ($conversions as $conversion) {
        ...
        expect($wouldScaleUp)->toBeBool()
            ->and($finalDimension)->toBeInstanceOf(Dimension::class);
        
        // Test conversion properties
        expect($conversion->getFormat())->toBe('mp4')
            ->and($conversion->getTargetBitrate())->toBeInt()
...
it('can simulate video processing with operation pipeline', function () {
    ...
    $editor
        ->trim(0, 60)  // First minute
        ->resize(new Dimension(1280, 720))  // Standardize to 720p
        ->convert(new Conversion720p());  // Apply quality conversion
    ...
    foreach ($operations as $operation) {
        expect($operation)->toBeArray()
            ->and(array_key_exists('type', $operation))->toBeTrue();
    }
