...
it('can create video editor using facade', function () {
    ...
    expect($editor)->toBeInstanceOf(VideoEditor::class)
        ->and($editor->getSourcePath())->toBe($videoPath);
});
...
it('can create video editor from URL using facade', function () {
    ...
    expect($editor)->toBeInstanceOf(VideoEditor::class)
        ->and($editor->getSourcePath())->toBe($videoUrl);
});
...
it('demonstrates facade usage patterns', function () {
    ...
    expect($localEditor)->toBeInstanceOf(VideoEditor::class)
        ->and($urlEditor)->toBeInstanceOf(VideoEditor::class)
        ->and($publicEditor)->toBeInstanceOf(VideoEditor::class);
    
    expect(count($localEditor->getOperations()))->toBe(2)
        ->and(count($urlEditor->getOperations()))->toBe(2)
        ->and(count($publicEditor->getOperations()))->toBe(2);
});
...
it('can use facade for batch processing', function () {
    ...
    foreach ($editors as $editor) {
        expect($editor)->toBeInstanceOf(VideoEditor::class)
            ->and(count($editor->getOperations()))->toBe(2);
    }
});
...
it('demonstrates advanced facade usage', function () {
    ...
    expect(count($operations))->toBe(5)
        ...
        ->and($operations[3]['type'])->toBe('frame_rate')
        ->and($operations[4]['type'])->toBe('convert');
