...
it('creates 144p conversion with correct dimensions', function () {
    ...
    expect($resolution->getDimension())
        ...
        ->and($resolution->getDimension()->getWidth())
        ->toBe(256)
        ->and($resolution->getDimension()->getHeight())
        ->toBe(144)
        ->and($resolution->getName())
        ->toBe('144p')
        ->and($resolution->getTargetBitrate())
        ->toBe(150);
});
...
it('conversion respects scale-up constraints', function () {
    ...
    expect($targetDimension->getWidth())
        ->toBe(3840)
        ->and($targetDimension->getHeight())
        ->toBe(2160);
...
it('all conversions have proper bitrate settings', function () {
    ...
    foreach ($resolutions as $resolution) {
        expect($resolution->getTargetBitrate())
            ->toBeInt()
            ->and($resolution->getTargetBitrate())
            ->toBeGreaterThan(0)
            ->and($resolution->getName())
            ->toBeString()
            ->and($resolution->getDimension())
            ->toBeInstanceOf(Dimension::class);
    }
});
