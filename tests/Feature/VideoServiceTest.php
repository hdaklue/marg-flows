...
it('can process video operations in correct order', function () {
    ...
    $editor
        ->trim(5, 30)        // Should be index 0
        ->resize(new Dimension(1280, 720))  // Should be index 1
        ->crop(0, 0, new Dimension(640, 480)); // Should be index 2
    
    $operations = $editor->getOperations();
    
    expect($operations[0]['type'])->toBe('trim')
        ->and($operations[1]['type'])->toBe('resize')
        ->and($operations[2]['type'])->toBe('crop');
});
...
it('correctly calculates dimension properties', function () {
    ...
    expect($dimension->getWidth())->toBe(1920)
        ->and($dimension->getHeight())->toBe(1080)
        ->and($dimension->getPixelCount())->toBe(2073600)
        ->and($dimension->getAspectRatio())->toBeInstanceOf(\App\Services\Video\ValueObjects\AspectRatio::class);
});
...
it('can scale dimensions correctly', function () {
    ...
    $scaledDimension = $dimension->scaleTo($targetDimension->getWidth(), $targetDimension->getHeight());
    
    expect($scaledDimension->getWidth())->toBe(960)
        ->and($scaledDimension->getHeight())->toBe(540);
});
...
it('correctly handles scaling scenarios', function () {
    ...
    expect($result->getWidth())->toBeLessThanOrEqual(1920)
        ->and($result->getHeight())->toBeLessThanOrEqual(1080)
        ->and($result->getPixelCount())->toBeGreaterThan($smallDimension->getPixelCount());
});