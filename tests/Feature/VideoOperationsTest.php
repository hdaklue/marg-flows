...
it('creates resize operation with correct metadata', function () {
    ...
    expect($operation->getName())->toBe('resize')
        ->and($operation->canExecute())->toBeTrue()
        ->and($operation->getMetadata())->toBeArray()
        ->and($operation->getMetadata()['width'])->toBe(1280)
        ->and($operation->getMetadata()['height'])->toBe(720);
});
...
it('creates crop operation with correct metadata', function () {
    ...
    expect($operation->getName())->toBe('crop')
        ->and($operation->canExecute())->toBeTrue()
        ->and($operation->getMetadata())->toBeArray()
        ->and($operation->getMetadata()['x'])->toBe(100)
        ->and($operation->getMetadata()['y'])->toBe(50)
        ->and($operation->getMetadata()['width'])->toBe(640)
        ->and($operation->getMetadata()['height'])->toBe(480);
});
...
it('creates trim operation with correct metadata', function () {
    ...
    expect($operation->getName())->toBe('trim')
        ->and($operation->canExecute())->toBeTrue()
        ->and($operation->getMetadata())->toBeArray()
        ->and($operation->getMetadata()['start'])->toBe(10.5)
        ->and($operation->getMetadata()['duration'])->toBe(60.0)
        ->and($operation->getMetadata()['end'])->toBe(70.5);
});
...
it('creates watermark operation with default parameters', function () {
    ...
    expect($operation->getName())->toBe('watermark')
        ->and($operation->getMetadata())->toBeArray()
        ->and($operation->getMetadata()['watermark_path'])->toBe('watermarks/logo.png')
        ->and($operation->getMetadata()['position'])->toBe('bottom-right')
        ->and($operation->getMetadata()['opacity'])->toBe(1.0);
});
...
it('operations maintain execution order through index', function () {
    ...
    expect($metadata[0]['operation'])->toBe('trim')
        ->and($metadata[0]['priority'])->toBe(0)
        ->and($metadata[1]['operation'])->toBe('resize')
        ->and($metadata[1]['priority'])->toBe(1)
        ->and($metadata[2]['operation'])->toBe('crop')
        ->and($metadata[2]['priority'])->toBe(2);
});
...
it('dimension operations work correctly', function () {
    ...
    expect($dimension->getAspectRatio())->toBeInstanceOf(\App\Services\Video\ValueObjects\AspectRatio::class);
    ...
    expect($scaled->getWidth())->toBe(960)
        ->and($scaled->getHeight())->toBe(540);
});