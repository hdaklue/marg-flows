...
final class ManageMembers extends Component implements HasActions, HasForms
{
    ...
    public function mount(
        null|RoleableEntity $roleableEntity,
        null|RoleableEntity $scopeToEntity,
    ) {
        if ($roleableEntity) {
            $this->canEdit = $this->authorize(
                'manageMembers',
                $roleableEntity,
            )->allowed();
    ...
    private function reloadData()
    {
        ...
        $this->dispatch(
            "roleable-entity:members-updated.{$this->roleableEntity->getKey()}",
        );
    }
}
