...
Route::prefix('chunks')
    ...
    ->group(function () {
        // Store chunk
        Route::post('upload', [ChunkedUploadController::class, 'store'])->name(
            'chunks.upload.store',
        );

        // Cleanup chunks
        Route::post('cleanup', [ChunkedUploadController::class, 'cleanup'])->name(
            'chunks.upload.cleanup',
        );

        // Delete chunk
        Route::delete('upload', [ChunkedUploadController::class, 'delete'])->name(
            'chunks.upload.delete',
        );

        // Cancel upload session
        Route::post('cancel', [ChunkedUploadController::class, 'cancel'])->name(
            'chunks.upload.cancel',
        );
    });

// Upload progress tracking
Route::prefix('upload/{sessionId}/progress')
    ->middleware(['auth'])
    ->group(function () {
        // Get upload progress
        Route::get('/', [UploadProgressController::class, 'show'])->name('chunks.progress.show');

        // Delete progress tracking
        Route::delete('/', [UploadProgressController::class, 'destroy'])->name(
            'chunks.progress.destroy',
        );
    });

// Chunk file management routes
Route::prefix('chunks/{tenantId}/files')
    ->middleware(['auth'])
    ->group(function () {
...
Route::get('chunks/{tenantId}/serve/{type}/{filename}', function (
    string $tenantId,
    string $type,
    string $filename,
) {
