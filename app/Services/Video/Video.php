...
final class Video
{
    ...
    private function validatePath(): void
    {
        if ($this->isUrl) {
            ...
            if (! Storage::disk($this->disk)->exists($this->path)) {
                throw new InvalidArgumentException("Video file not found: {$this->path} on disk: {$this->disk}");
            }
        }
    }
    ...
    private function loadVideoMetadata(): void
    {
        ...
        try {
            ...
            throw new InvalidArgumentException("Could not analyze video: {$this->path}. Error: " . $e->getMessage());
        }
    }
