...
export default function chunkedFileUploadComponent(config) {
    return {
        ...
        async uploadFileDirectly(entry) {
            ...
            const formData = this.buildFormData(file, entry.id, { direct: true });
        ...
        getFileIcon(file) {
            ...
            if (type.startsWith('image/')) return 'heroicon-o-photo';
            if (type.startsWith('video/')) return 'heroicon-o-video-camera';
            if (type.startsWith('audio/')) return 'heroicon-o-musical-note';
            if (type.includes('pdf')) return 'heroicon-o-document-text';
            if (type.includes('word') || type.includes('document')) return 'heroicon-o-document';
            if (type.includes('excel') || type.includes('spreadsheet')) return 'heroicon-o-table-cells';
            if (type.includes('zip') || type.includes('archive')) return 'heroicon-o-archive-box';
