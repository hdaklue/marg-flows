<div x-load x-data="designAnnotationApp('{{ $image }}', @js($comments), @js($this->getImageWidth()), @js($this->getImageHeight()), @js($this->getImageMetadataForJs()))" @keydown.arrow-up.window.prevent="handleArrowKeys($event)"
    ...
    class="min-w-screen flex min-h-screen" :id="$id('x-mentionable-text')">
        ...
        <div
            ...
            <div class="relative" @click.outside="showCommentFilter = false">
                ...
                <div x-show="showCommentFilter" x-cloak x-transition:enter="transition ease-out duration-200"
                    ...
                    class="absolute bottom-full left-1/2 z-50 mb-2 hidden w-56 -translate-x-1/2 space-y-1 rounded-lg border border-zinc-200 bg-white p-3 shadow-lg sm:block dark:border-zinc-700 dark:bg-zinc-800"
            ...
            <div class="flex items-center gap-1">
                <button @click="zoomOut()" @touchstart.passive @touchend.passive="zoomOut()" :disabled="!canZoomOut"
                    ...
                    class="flex h-10 w-10 items-center justify-center rounded-lg bg-zinc-100 text-zinc-700 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-sky-500 sm:h-8 sm:w-8 dark:bg-zinc-800 dark:text-zinc-300"
                ...
                <template x-if="isMobile && gestureSequence.length === 0">
                    <div class="ml-2 hidden text-xs text-zinc-500 sm:block dark:text-zinc-400">
                        Pinch to zoom
                    </div>
                </template>
            </div>
