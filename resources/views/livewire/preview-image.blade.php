<div x-data="designAnnotationApp('{{ $image }}', @js($comments), @js($this->getImageWidth()), @js($this->getImageHeight()), @js($this->getImageMetadataForJs()))" @keydown.arrow-up.window.prevent="handleArrowKeys($event)"
    ...
    class="flex min-h-screen min-w-screen" :id="$id('x-mentionable-text')">

    @livewire('feedback.create-feedback-modal')

    <div
        class="flex flex-col items-center justify-between min-w-full min-h-screen mx-auto overflow-hidden bg-white border rounded-lg border-zinc-200 dark:border-zinc-700 dark:bg-zinc-950">
        <!-- Loading State -->
        <div x-show="!imageReady" x-cloak class="flex items-center justify-center h-64">
        ...
        <!-- Image Viewport -->
        <div class="flex flex-col items-center justify-center w-full h-full grow-1">
                    ...
                    <div x-ref="mainContainer"
                        ...
                        <div x-ref="innerWrapper"
                            ...
                            <template x-if="isSelecting">
                                <div class="absolute transition-all duration-150 border-2 border-blue-400 border-dashed rounded pointer-events-none animate-pulse bg-blue-400/20"
                                    ...
                                    <div class="absolute w-2 h-2 bg-blue-500 rounded-full -right-1 -top-1"></div>
                                    <div class="absolute w-2 h-2 bg-blue-500 rounded-full -bottom-1 -left-1"></div>
                                    <div class="absolute w-2 h-2 bg-blue-500 rounded-full -bottom-1 -right-1"></div>
                                </div>
                            </template>
                        </div>
                        <!-- End Inner Wrapper -->
                        ...
                        <div x-show="showSelectionMode" x-transition:enter="transition ease-out duration-200"
                            ...
                            class="absolute z-30 px-4 py-2 text-sm text-white transform -translate-x-1/2 bg-blue-500 rounded-full shadow-lg left-1/2 top-4">
                        ...
                        <div x-show="isMobile && isZoomed && currentGesture === null && !showSelectionMode"
                            ...
                            class="absolute px-3 py-2 text-xs text-white transform -translate-x-1/2 border rounded-lg shadow-lg bottom-20 left-1/2 border-gray-700/50 bg-gray-900/80 backdrop-blur-sm">
                            <div class="flex items-center gap-2">
                        ...
                        <div x-show="!isMobile && showModeIndicator"
                            ...
                            x-transition:leave="transition ease-in duration-150"
                            x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95"
                            class="absolute z-30 px-3 py-2 text-sm font-medium border rounded-lg shadow-lg right-4 top-4 backdrop-blur-sm"
                            :class="{
                        ...
                        <div x-show="currentGesture && window.location.hostname === 'localhost'"
                            class="absolute px-2 py-1 font-mono text-xs text-white border border-gray-600 rounded right-4 top-4 bg-black/70">
        ...
        <div
            class="flex items-center justify-center w-full h-12 gap-3 px-4 py-3 bg-white border-t rounded-b-lg grow-0 place-self-end border-zinc-200 dark:border-zinc-700 dark:bg-zinc-900">
            <!-- Comment Filter -->
            <div class="relative" @click.outside="showCommentFilter = false">
                ...
                <div x-show="showCommentFilter" x-cloak x-transition:enter="transition ease-out duration-200"
                    ...
                    class="absolute z-50 hidden w-56 p-3 mb-2 space-y-1 -translate-x-1/2 bg-white border rounded-lg shadow-lg bottom-full left-1/2 border-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 sm:block"
            ...
            <div class="flex items-center gap-1">
                ...
                <template x-if="isMobile && gestureSequence.length === 0">
                    <div class="hidden ml-2 text-xs text-zinc-500 dark:text-zinc-400 sm:block">
                        Pinch to zoom
                    </div>
                </template>
            </div>
