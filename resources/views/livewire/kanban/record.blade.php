<div x-data="{
    ...
    @if ($shouldShowProgressDetails)
        <div class="mt-2 flex flex-row flex-wrap gap-x-2 dark:text-gray-400">
            ...
            <div class="mt-2 cursor-default">
                <p class="text-2xs w-full text-gray-300" x-show="progressDetails.days_remaining > 0"
        ...
        <div x-data class="flex justify-end pt-2 text-sm">
            <div x-data>
                <button title="Manage members"
                    ...
                    {{-- wire:click="recordClicked('{{ $record->getKey() }}', {{ @json_encode($record) }})" --}} {{-- @click.prevent="$dispatch('open-modal',{id: 'edit-members-modal-{{ $record->getKey() }}'})" --}}
