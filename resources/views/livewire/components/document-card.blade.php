<div>
    <div x-data="{
        ...
        <div class="flex h-auto flex-col justify-between gap-y-1 border-t border-zinc-700/30 p-2">
            <div class="relative h-8">
                ...
                <div x-show="!editing" x-cloak
                    ...
                    <!-- Loading indicator -->
                    <div x-show="loading" x-cloak class="absolute -end-1 -top-1">
            ...
            <p class="text-2xs self-baseline text-zinc-500">{{ __('document.labels.last_update') }}: {{ $updatedAt }}</p>
        ...
        <div x-show="showMenu && !isTouchDevice" x-cloak x-ref="menu"
            ...
            <button @if (!$this->userPermissions['canEdit']) style="display: none;" @endif @click="startEdit(); closeMenu()"
                ...
                {{ __('document.context_menu.rename') }}
            </button>
            ...
            <button @click="console.log('duplicate'); closeMenu()"
                ...
                {{ __('document.context_menu.duplicate') }}
            </button>
        ...
        <div x-show="showMenu && isTouchDevice" x-cloak
            ...
            <div class="pb-safe w-full max-w-sm rounded-t-xl bg-white dark:bg-zinc-800">
                <div class="flex items-center justify-between border-b px-4 py-3 dark:border-zinc-700">
                    <h3 class="font-semibold dark:text-zinc-300">{{ __('document.context_menu.page_options') }}</h3>
                ...
                <div class="py-2">
                    <button @if (!$this->userPermissions['canEdit']) style="display: none;" @endif
                        ...
                        <svg class="me-3 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                        {{ __('document.context_menu.rename') }}
                    ...
                    <button @click="console.log('delete'); closeMenu()"
                        ...
                        {{ __('document.context_menu.delete') }}
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="flex w-full justify-end py-2">
        ...
        <x-user-avatar-stack :users="$this->participantsArray" :roleableKey="$this->page->getKey()" :roleableType="$this->page->getMorphClass()" :scopeToType="$this->page->documentable->getMorphClass()" :scopeToKey="$this->page->documentable->getKey()"
            :canEdit="$this->userPermissions['canManageMembers']" size='2xs' />
    </div>
</div>
