<div class="relative" x-data="{ open: false }" @language-changed.window="window.location.reload()">
    <button 
        @click="open = !open"
        ...
        <div class="py-1">
            @foreach($availableLocales as $locale => $label)
                <button 
                    ...
                    @click="open = false"
                    class="flex items-center w-full px-4 py-2 text-sm text-left hover:bg-gray-100 dark:hover:bg-gray-700 {{ $currentLocale === $locale ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-gray-700 dark:text-gray-200' }}"
                >
