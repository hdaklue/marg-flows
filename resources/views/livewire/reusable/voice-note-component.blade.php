<div>
    ...
    @if (!empty($this->voiceNoteUrls))

        <div class="mb-3 space-y-2">
            @foreach ($this->voiceNoteUrls as $index => $url)
                <div class="flex items-center gap-2" wire:key="voice-note-{{ md5($url) }}">
                    <div class="flex-1">
                        <x-audio-player :audio-url="$url" size="sm" :outlined="false" />
                    </div>
    ...
    <div wire:loading.remove wire:target="addVoiceNote">
        @if ($this->canAcceptNotes)
            <!-- Voice Recorder Interface (Alpine.js) -->
            <x-voice-recorder :on-submit="'(blob) => $wire.call(\'addVoiceNote\', blob)'" />
        @else
            ...
            <div
                ...
                <span class="text-sm text-amber-700 dark:text-amber-300">Maximum 3 voice notes reached. Remove one to
                    add more.</span>
            </div>
        @endif
    </div>
