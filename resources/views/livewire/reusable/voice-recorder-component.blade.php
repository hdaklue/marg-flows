...
class="relative flex w-full items-center rounded-lg border border-zinc-300 p-1 transition-all duration-300 dark:border-zinc-600"
:class="{
...
x-init="init()">
    ...
    <div class="grow-0">
        <button @click="isRecording ? stopRecording() : startRecording()"
            :disabled="!isSupported || isUploading || !isInitialized"
            ...
            :class="{
                'bg-emerald-500 hover:bg-emerald-600': !isRecording && isSupported && !isUploading && isInitialized,
                'bg-red-500 hover:bg-red-600 animate-pulse': isRecording,
                'bg-amber-500': isUploading,
                'bg-zinc-400 cursor-not-allowed': !isSupported || isUploading || !isInitialized
            }">
            ...
            <svg x-show="!isRecording && !isUploading" class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                ...
                <path d="M6 10.5a.75.75 0 0 1 .75.75 5.25 5.25 0 0 0 10.5 0 .75.75 0 0 1 1.5 0 6.751 6.751 0 0 1-6 6.709v2.291h3a.75.75 0 0 1 0 1.5h-7.5a.75.75 0 0 1 0-1.5h3v-2.291A6.751 6.751 0 0 1 5.25 11.25.75.75 0 0 1 6 10.5Z" />
            </svg>
        ...
        <div x-show="hasRecording && !isRecording" class="w-full">
            <div class="flex items-center gap-2">
                ...
                <button @click="togglePlayback()"
                    ...
                    :class="{
                        'border-sky-500 text-sky-500 hover:bg-sky-50 dark:hover:bg-sky-950': !isPlaying,
    ...
    <div x-show="hasRecording && !isRecording" class="flex items-center gap-1">
        ...
        <button @click="submitRecording()" :disabled="isUploading"
            ...
            :class="{
                ...
                'text-zinc-400 cursor-not-allowed': isUploading
            }">
        ...
        <button @click="deleteRecording()"
            class="flex items-center justify-center p-1 transition-colors duration-200 rounded text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
    ...
    <div class="text-xs whitespace-nowrap ml-2">
        <!-- Timer (when recording) -->
        <div x-show="isRecording" 
            :class="{
                'text-red-500 dark:text-red-400 font-medium': currentDuration >= maxDuration * 0.9,
                'text-amber-500 dark:text-amber-400 font-medium': currentDuration >= maxDuration * 0.75 && currentDuration < maxDuration * 0.9,
        ...
        <div x-show="!isRecording && !hasRecording && isSupported" 
