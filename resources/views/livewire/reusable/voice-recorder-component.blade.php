    ...
    <div x-load x-data="recorder({
        ...
        <div class="grow-0">
            <button @click="isRecording ? stopRecording() : startRecording()" :disabled="!isSupported || isUploading"
                ...
                <svg x-show="!isRecording && !isUploading" fill="currentColor" viewBox="0 0 24 24"
                    :class="{
                        'w-3 h-3': size === 'sm',
                        'w-4 h-4': size === 'md',
                        'w-5 h-5': size === 'lg'
                ...
                <!-- Upload Spinner -->
                <svg x-show="isUploading" class="animate-spin" fill="none" viewBox="0 0 24 24"
                    :class="{
                        'w-3 h-3': size === 'sm',
                        'w-4 h-4': size === 'md',
                        'w-5 h-5': size === 'lg'
        ...
        <div x-show="isRecording || hasRecording" x-transition:enter="transition ease-out duration-200"
            ...
            x-transition:leave-end="opacity-0" class="flex-1" x-cloak
            :class="{
                'mx-2': size === 'sm',
                'mx-3': size === 'md',
                'mx-4': size === 'lg'
            ...
            <div x-show="isRecording" class="w-full">
                <div x-ref="recordingWaveform" class="w-full rounded bg-zinc-50 dark:bg-zinc-800"
                    :class="{
                        'h-6': size === 'sm',
                        'h-8': size === 'md',
                        'h-10': size === 'lg'
            ...
            <div x-show="hasRecording && !isRecording" class="w-full">
                <div class="flex items-center gap-2">
                    ...
                    <button @click="togglePlayback()"
                        ...
                        <!-- Play Icon -->
                        <svg x-show="!isPlaying" class="ml-0.5" fill="currentColor" viewBox="0 0 24 24"
                            :class="{
                                'h-2.5 w-2.5': size === 'sm',
                                'h-3 w-3': size === 'md',
                                'h-3.5 w-3.5': size === 'lg'
        ...
        <div x-show="hasRecording && !isRecording" class="flex items-center"
            :class="{
                'gap-1': size === 'sm',
                'gap-1': size === 'md',
                'gap-2': size === 'lg'
        ...
        <div class="whitespace-nowrap ps-1"
            :class="{
                'ml-1.5 text-xs': size === 'sm',
                'ml-2 text-xs': size === 'md',
                'ml-2.5 text-sm': size === 'lg'
            }">
            ...
            <div x-show="!isRecording && !hasRecording && isSupported" class="text-zinc-500 dark:text-zinc-400"
                x-cloak>
