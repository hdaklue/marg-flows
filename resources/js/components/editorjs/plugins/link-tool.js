 ...
 * @property {boolean} enablePreview — whether preview is enabled for this block
 */
...
export default class LinkTool {
  ...
  /**
   * @param {object} options - Tool constructor options from Editor.js
  ...
  constructor({ data, config, api, readOnly }) {
    ...
    // Store enablePreview as instance property, not in shared config
    // Use saved data first, then fall back to global config
    this.enablePreview = data.enablePreview !== undefined 
      ? data.enablePreview 
      : (config.enablePreview !== false); // Default to true
  ...
  render() {
    ...
    /**
     * If Tool already has data, render appropriately based on saved setting, otherwise insert input
     */
    if (this.data.link) {
      // Use instance-specific enablePreview setting
      const shouldShowPreview = this.enablePreview;
        
      if (shouldShowPreview && Object.keys(this.data.meta).length) {
        this.nodes.container.appendChild(this.nodes.linkContent);
        this.showLinkPreview(this.data.meta);
      } else {
        this.showSimpleLink(this.data.link);
      }
    } else {
  ...
  set data(data) {
    this._data = Object.assign({}, {
      ...
      enablePreview: data.enablePreview !== undefined ? data.enablePreview : this._data.enablePreview,
    });
  }
   ...
   *
   * @param {PasteEvent|KeyboardEvent} event - fetching could be fired by a paste or keydown events
   */
  startFetching(event) {
  ...
  async fetchLinkData(url) {
    ...
    try {
      const response = await fetch(`${this.config.endpoint}?url=${encodeURIComponent(url)}`, {
        method: 'GET',
   ...
   * Excludes YouTube URLs (handled by video-embed plugin) and file patterns
   * 
   * @returns {object}
   */
  static get pasteConfig() {
    return {
      patterns: {
        // Match URLs but exclude YouTube and file extensions
        url: /^https?:\/\/(?!(?:www\.)?(?:youtube\.com|youtu\.be))(?:[^\s]*(?<!\.(mp4|webm|ogg|mov|avi|wmv|flv|mkv|jpg|jpeg|png|gif|svg|pdf|doc|docx|xls|xlsx|ppt|pptx|txt|zip|rar|tar|gz|mp3|wav|ogg|flac)))/i
      }
    };
  }
  ...
  /**
   * Handle paste events
   * Only processes URLs that don't match YouTube patterns or file extensions
   * 
   * @param {object} event - paste event from Editor.js
   */
  onPaste(event) {
    if (event.type === 'pattern' && event.detail.key === 'url') {
      const url = event.detail.data.trim();
      
      // Double-check to exclude YouTube URLs (safety check)
      const youtubePattern = /^https?:\/\/(?:www\.)?(youtube\.com\/watch\?v=|youtu\.be\/).*/i;
      if (youtubePattern.test(url)) {
        return;
      }
      
      // Double-check to exclude file extensions
      const filePattern = /\.(mp4|webm|ogg|mov|avi|wmv|flv|mkv|jpg|jpeg|png|gif|svg|pdf|doc|docx|xls|xlsx|ppt|pptx|txt|zip|rar|tar|gz|mp3|wav|ogg|flac)$/i;
      ...
      if (this.enablePreview) {
        ...
        // If preview is disabled, just store the URL without fetching metadata
        this.showSimpleLink(url);
      }
    }
  }

  /**
   * Show a simple link without preview (when preview is disabled)
   * 
  ...
  showSimpleLink(url) {
    ...
    // Create simple link display
    this.nodes.container.innerHTML = '';
    const linkElement = this.make('a', this.CSS.linkContent, {
      href: url,
      target: '_blank',
      rel: 'nofollow noindex noreferrer'
    });
    
    // Show hostname instead of full URL for cleaner display
    try {
    ...
    this.nodes.container.appendChild(linkElement);
  }
}